...
const btn = (active=true) => ({ width:"100%", marginTop:"14px", padding:"13px", background: active?"linear-gradient(135deg,#5a3a00,#3a2000)":"rgba(30,15,5,.4)", border:"1px solid "+(active?"#c8943a":"rgba(200,148,58,.1)"), borderRadius:"8px", color:active?"#f4c842":"#4a3020", fontFamily:"'Cinzel',serif", fontSize:"13px", letterSpacing:"2px", cursor:active?"pointer":"not-allowed", transition:"all .2s" });
...
export default function Lobby({ onJoin }) {
  ...
  const createSession = async () => {
    ...
    const { error: err } = await supabase.from("sessions").insert({
      ...
      players: {}, initiative: [], current_turn: null,
  ...
  if (mode === "hosting") return (
    <div style={wrap}><div style={card}>
      <div style={{fontFamily:"'Cinzel',serif",fontSize:"22px",fontWeight:700,background:"linear-gradient(135deg,#f4c842,#e8a020,#f4c842)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",textAlign:"center",letterSpacing:"3px",marginBottom:"4px"}}>⚔️ SESSION READY</div>
      <div style={{textAlign:"center",color:"#5a4025",fontFamily:"'Cinzel',serif",fontSize:"10px",letterSpacing:"2px",marginBottom:"20px"}}>SHARE THIS CODE</div>
      ...
      <div style={{color:"#7a6040",fontSize:"14px",textAlign:"center",lineHeight:1.6,marginBottom:"8px"}}>Friends open the site, click Join Session, enter this code and their name. Expecting <strong style={{color:"#c8943a"}}>{playerCount} player{playerCount>1?"s":""}</strong> total.</div>
      <button style={btn()} onClick={()=>onJoin({sessionId:sessionCode,playerName:name.trim(),isHost:true,setting,personality,playerCount})}>⚔️ ENTER THE DUNGEON</button>
    </div></div>
  );

  if (mode === "host") return (
    <div style={wrap}><div style={card}>
      <div style={{fontFamily:"'Cinzel',serif",fontSize:"20px",fontWeight:700,background:"linear-gradient(135deg,#f4c842,#e8a020)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",textAlign:"center",letterSpacing:"3px",marginBottom:"20px"}}>⚔️ HOST SESSION</div>
  ...
  if (mode === "join") return (
    <div style={wrap}><div style={card}>
      <div style={{fontFamily:"'Cinzel',serif",fontSize:"20px",fontWeight:700,background:"linear-gradient(135deg,#f4c842,#e8a020)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",textAlign:"center",letterSpacing:"3px",marginBottom:"20px"}}>🗝️ JOIN SESSION</div>
      <label style={lbl}>YOUR NAME</label>
